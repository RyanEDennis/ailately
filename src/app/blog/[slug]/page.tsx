...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  if (!piece) return {};
  const url = `${SITE.url}/blog/${piece.slug}`;
  return {
    title: `${piece.seo.title ?? piece.title} · Opinion`,
    description: piece.seo.description ?? piece.dek,
    keywords: piece.seo.keywords,
    alternates: { canonical: url },
    openGraph: { type: "article", url, title: piece.title, description: piece.dek, siteName: SITE.name, authors: [piece.author], section: "Opinion", tags: piece.tags },
